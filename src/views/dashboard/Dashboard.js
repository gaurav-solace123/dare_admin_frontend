  ...
  const getCreditsActivatedByLevel = async () => {
    try {
      ...
      if (result?.success) {
        ...
        const temp = [
          { value: response?.middleSchool, label: "Middle School", color: "#5cb7fc", percentage: response?.middleSchool },
          { value: response?.elementary, label: "Elementary School", color: "#fecc6c", percentage: response?.elementary },
        ];
